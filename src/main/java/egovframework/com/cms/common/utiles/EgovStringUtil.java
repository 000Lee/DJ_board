...
package egovframework.com.cms.common.utiles;
...
public class EgovStringUtil {
    ...
    public static final String[] arrNum={"","일","이","삼","사","오","육","칠","팔","구"};
    public static final String[] arrUnit= {"","십","백","천","만","십만","백만","천만", "억","십억","백억","천억","조","십조","백조"};
...
*/
    ...
    public static String moneyToHan(String val) {
        ...
        char[] arrMoney = val.toCharArray() ;
        int size = arrMoney.length;
        int code = size ;
        String tmpUnit = "" ;
        int numInx = 0 ;
        int nextNumInx = 0 ;
        ...
        for(int inx= 0 ; inx < size ;inx++) {
            code--;
            numInx = Integer.parseInt(arrMoney[inx]+"" );
            tmpUnit = "" ;
            if(!arrNum[numInx].equals("") ){
                tmpUnit = arrUnit[code];
                if(code>4) {
                    nextNumInx = Integer.parseInt(arrMoney[inx+1]+"" ) ;
                    dxNumInx = Integer.parseInt(arrMoney[inx+2]+"");
                    txNumInx = Integer.parseInt(arrMoney[inx+3]+"");

                    if(( Math.floor(code/4) == Math.floor((code-1)/4) && !arrNum[nextNumInx].equals(""))
                            || ( Math.floor(code/4) == Math.floor((code-2)/4) && !arrNum[dxNumInx].equals(""))
                            || ( Math.floor(code/4) == Math.floor((code-3)/4) && !arrNum[txNumInx].equals("")) ) {

                        tmpUnit=arrUnit[code].substring(0,1);
                    }
                }
            }
            money.append( arrNum[numInx]+tmpUnit );
        }

        return minus+money.toString() ;
    }
}
