  ...
  function fnUploadFile(e) {
    ...
    if(attFiles.length > 0){
      ...
      for (var i = 0; i < attFiles.length; i++) {
        ...
        html += '   <td>' + attFiles[i].size + '</td>';
        html += '   <td>';
        html += '       <input type="button" style="width:70%; height:25px;margin-top: 5px;" onclick="fnUploadFile(' + i + ')" value="삭제">'
        html += '   </td>';
        html += '</tr>';
      }
      ...
      if($("#fileGrid").find("tr").length == 0){
        $("#fileGrid").html('<tr>' +
                '	<td colspan="4" style="text-align: center;height:40px;">선택된 파일이 없습니다.</td>' +
                '</tr>');
      }
    }
