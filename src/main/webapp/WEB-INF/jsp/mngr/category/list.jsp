...
<head>
	<jsp:include page="/WEB-INF/jsp/mngr/template/header.jsp" flush="true">
		<jsp:param value="CATEGORY LIST" name="headerTitle"/>
	</jsp:include>
</head>
<body>
			...
			<div class="form-inline text-right">
				<div class="form-group col-sm2">
					<select class="form-control" name="categoryGroup" id="categoryGroup">
						...
						<option value="${item.categoryGroup}" <c:if test="${categoryGroup == item.categoryGroup}">selected</c:if>>${item.categoryName}</option>
						</c:forEach>
					</select>
				</div>
				...
				<table class="table cmsTable">
					...
					<thead>
	              	...
	              	<tbody>
	              		...
	              		<tr>
	              			<td><c:forEach var="i" begin="1" end="${item.getTempIntField1()}">▷</c:forEach>▶${item.categoryName}</td>
	              			<td class="text-center">${item.categoryGroup}</td>
	              			<td class="text-center">${item.getStrUseAt()}</td>
	              			<td class="text-center"><button type="button" class="btn btn-default" onclick="fn_copyClipboard('${item.categoryUUID}')">키복사</button></td>
	              			<td class="text-center"><button type="button" class="btn btn-default" onclick="fn_childRegister('${item.categoryUUID}')">＋ 하위메뉴등록</button></td>
	              			<td class="text-center"><button type="button" class="btn btn-primary" onclick="fn_update('${item.categoryUUID}')">수정</button></td>
	              			<td class="text-center"><button type="button" class="btn btn-danger" onclick="fn_delete('${item.categoryUUID}')">삭제</button></td>
	              		</tr>
	              		</c:forEach>
		                <tr>
			                <td>▶카테고리 테스트</td>
	...
	function fn_search(){
		...
		comSubmit.addParam("categoryGroup",$("#categoryGroup").val());
	...
	function fn_register(){
		var comSubmit = new ComSubmit();
		comSubmit.setUrl("/mngr/category/register.do");
	...
	function fn_update(uuid){
		...
		comSubmit.addParam("categoryUUID",uuid);
		comSubmit.addParam("categoryGroup",${categoryGroup});
		comSubmit.addParam("searchContent",'${searchContent}');
		comSubmit.submit();
	}
	...
	function fn_delete(uuid){
		if(confirm("카테고리를 삭제하시겠습니까?")){
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("/mngr/category/delete.do");
			comSubmit.addParam("categoryUUID",uuid);
			comSubmit.addParam("categoryGroup",${categoryGroup});
			comSubmit.addParam("searchContent",'${searchContent}');
			comSubmit.submit();
