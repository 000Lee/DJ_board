...
<head>
	<jsp:include page="/WEB-INF/jsp/mngr/template/header.jsp" flush="true">
		<jsp:param value="CODE LIST" name="headerTitle"/>
	</jsp:include>
<body>
			...
			<div class="form-inline text-right">
				<div class="form-group col-sm2">
					<select class="form-control" name="searchColumn" id="searchColumn">
						...
						<option value="code" <c:if test="${searchColumn eq 'code'}">selected</c:if>>코드그룹</option>
						<option value="codeNm" <c:if test="${searchColumn eq 'codeNm'}">selected</c:if>>코드그룹명</option>
					</select>
				</div>
			...
			<div class="table-responsive">
				<table class="table cmsTable">
	            	...
	            	<thead>
	              	...
	              	<c:forEach var="item" items="${list}" varStatus="status">
	              		 <tr>
			                ...
			                <td class="text-center"><button type="button" class="btn btn-primary" onclick="fn_update(${item.codeIdx})">수정</button></td>
			                <td class="text-center"><button type="button" class="btn btn-danger" onclick="fn_delete(${item.codeIdx})">삭제</button></td>
		                </tr>
	              	</c:forEach>
	                ...
	                <tr>
		                ...
		                <td class="text-center">테스트 코드2</td>
	                ...
	                <tr>
		                ...
		                <td class="text-center">테스트 코드1</td>
		                <td class="text-center">사용함</td>
		                <td class="text-center"><button type="button" class="btn btn-primary" onclick="fn_update(${item.codeIdx})">수정</button></td>
		                <td class="text-center"><button type="button" class="btn btn-danger" onclick="fn_delete(${item.codeIdx})">삭제</button></td>
	                </tr>
	...
	function fn_write(){
		...
		comSubmit.setUrl("/mngr/code/register.do");
	...
	function fn_update(idx){
		...
		comSubmit.addParam("codeGroupIdx",${codeGroupIdx});
		comSubmit.addParam("codeIdx",idx);
		comSubmit.addParam("searchColumn",'${searchColumn}');
		comSubmit.addParam("searchContent",'${searchContent}');
		comSubmit.addParam("page",${page});
		comSubmit.addParam("rowCount",${rowCount});
	...
	function fn_delete(idx){
		if(confirm("상품을 삭제하시겠습니까?")){
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("/mngr/code/delete.do");
			comSubmit.addParam("codeGroupIdx",${codeGroupIdx});
			comSubmit.addParam("codeIdx",idx);
			comSubmit.addParam("searchColumn",'${searchColumn}');
			comSubmit.addParam("searchContent",'${searchContent}');
