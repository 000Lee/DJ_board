var fCommon = {
    ...
    addFileInfoTable : function(){
        ...
        if(fCommon.global.attFiles.length > 0){
            ...
            for (var i = 0; i < fCommon.global.attFiles.length; i++) {
                ...
                html += '   <td>' + (fCommon.global.attFiles[i].size.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')) + '</td>';
                html += '   <td>';
                html += '       <input type="button" value="삭제" style="width:70%; height:25px; margin-top: 5px;" onclick="fCommon.fnUploadFile(' + i + ')">'
                html += '   </td>';
                html += '</tr>';
            }
    ...
    fnUploadFile : function(e) {
        ...
        if(fCommon.global.attFiles.length > 0){
            ...
            var html = '';
            for (var i = 0; i <fCommon.global.attFiles.length; i++) {
                html += '<tr style="text-align: center; width:70%; height:25px;" class="addFile">';
                html += '   <td>' + fCommon.global.attFiles[i].name.split(".")[0] + '</td>';
                html += '   <td>' + fCommon.global.attFiles[i].name.split(".")[1] + '</td>';
                html += '   <td>' + (fCommon.global.attFiles[i].size.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')) + '</td>';
                html += '   <td>';
                html += '       <input type="button" value="삭제" style="width:70%; height:25px;" onclick="fCommon.fnUploadFile(' + i + ')">';
                html += '   </td>';
                html += '</tr>';
            }
